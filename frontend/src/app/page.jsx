...
const StatCard = ({ title, value, icon, link, color }) => (
  <Link
    href={link}
    className={`${color} p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow border border-gray-100`}
  >
...
const QuickAction = ({ title, description, link, icon }) => (
  <Link
    href={link}
    className="bg-white p-4 rounded-xl border border-gray-100 hover:border-green-500 hover:shadow-md transition-all"
  >
  ...
  return (
    <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-5 rounded-xl text-white">
      ...
      <div className="flex gap-4 mt-4 text-sm flex-wrap">
        ...
        <span title={t("weather.wind")}>
          🌬️ {Math.round(weather.current.wind)} km/h
        </span>
        <span title={t("weather.uvIndex")}>☀️ UV {weather.current.uv}</span>
        {weather.current.rain !== undefined && (
          <span title={t("weather.precipitation")}>
            🌧️ {weather.current.rain}mm
          </span>
        )}
      </div>
  ...
  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-md">
    ...
    {loading ? (
          ...
          {prices.slice(0, 5).map((price, i) => (
            ...
            >
              ...
              <div className="text-right">
                ...
                {price.change && (
                  <p
                    className={`text-xs ${price.change > 0 ? "text-green-500" : "text-red-500"}`}
                  >
      ...
      <div className="text-center py-4 text-gray-500">
        ...
        <Link href="/marketplace" className="text-green-600 text-sm">
          {t("market.viewMarketplace")} →
        </Link>
      </div>
    )}
  </div>
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-md">
      ...
      {pendingTasks.length > 0 ? (
        ...
        <div className="text-center py-4 text-gray-500">
          <p>{t("tasks.noTasks")}</p>
          <Link href="/tasks" className="text-green-600 text-sm">
            + {t("tasks.createTask")}
          </Link>
        </div>
      )}
...
const HomePage = () => {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          async () => {
            ...
            await fetchWeatherData(28.6139, 77.209);
  ...
  const fetchMarketPrices = async () => {
    ...
    try {
      const pricesRes = await marketPricesAPI
        .getAll()
        .catch(() => ({ rows: [] }));
