...
const ManPower = () => {
	...
	const positionOptions = ['Worker', 'Manager', 'Supervisor', 'Driver', 'Helper'];
	...
	const handleSave = async () => {
		...
		try {
			...
			setFormData({
				person_id: '', name: '', phone: '', position: '', description: '',
			});
			fetchWorkers();
		} catch (err) {
	...
	return (
		<main className="form-container">
			...
			{workers.length > 0 && (
				<div className="mt-8">
					<h2 className="text-xl font-bold text-gray-800 mb-4">Your Workers ({workers.length})</h2>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{workers.map((w) => (
							<div key={w.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-md hover:shadow-lg transition-shadow">
								<div className="flex justify-between items-start">
									<div>
										<h3 className="font-bold text-lg text-gray-800">{w.name}</h3>
										<p className="text-gray-600 mt-1">ID: <span className="font-semibold">{w.person_id || 'N/A'}</span></p>
										<p className="text-gray-600">Position: <span className="font-semibold">{w.position || 'N/A'}</span></p>
										{w.phone && <p className="text-gray-600">Phone: <span className="font-semibold">{w.phone}</span></p>}
									</div>
									<button 
