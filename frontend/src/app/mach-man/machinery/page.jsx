		...
		<main className="form-container">
			...
			<div className="form-btns-container mt-6">
				<button 
					...
					onClick={() => setFormData({ machine_id: '', description: '', total_number: '' })}
				>
					Clear
				</button>
				<button 
			...
			{machinery.length > 0 && (
				<div className="mt-8">
					<h2 className="text-xl font-bold text-gray-800 mb-4">Your Machinery ({machinery.length})</h2>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{machinery.map((m) => (
							<div key={m.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-md hover:shadow-lg transition-shadow">
								<div className="flex justify-between items-start">
									<div>
										<h3 className="font-bold text-lg text-gray-800">{m.machine_id}</h3>
										<p className="text-gray-600 mt-1">{m.description || 'No description'}</p>
										<p className="text-gray-600">Quantity: <span className="font-semibold">{m.total_number}</span></p>
									</div>
									<button 
