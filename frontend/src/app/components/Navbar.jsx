...
const getNavLinks = (t) => [
  ...
  },
  {
    ...
    options: [{ title: t("nav.machinery"), path: "/mach-man/machinery" }],
  },
];
...
const NavItem = ({ title, path, options }) => {
  if (!path) {
    return (
      <div className="relative group text-sm font-semibold py-3">
        <button className="w-full h-full px-3 text-gray-700 hover:text-green-600 transition-colors">
          {title}
        </button>
        <ul
          className="w-max min-w-[160px] list-none hidden group-hover:flex hover:flex flex-col 
  ...
  return (
    <Link
      href={path}
      className="text-sm font-semibold px-3 py-3 text-gray-700 hover:text-green-600 transition-colors"
    >
      {title}
    </Link>
  );
...
const NavItemMobile = ({ title, path, options }) => {
  ...
  return (
    <Link
      href={path}
      className="px-3 py-2 font-semibold text-gray-800 hover:text-green-600 transition-colors"
    >
      {title}
    </Link>
  );
...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex justify-center items-center bg-white border-b border-gray-100">
      {/* Desktop Navigation */}
      <ul
        className="hidden md:flex items-center gap-2 justify-center 
